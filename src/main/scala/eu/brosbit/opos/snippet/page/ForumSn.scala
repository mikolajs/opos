  ...
  def showThreadsTable() = {
    ...
    "#tagsInfo *" #> S.param("tags").openOr(" brak ") &
      ...
      "#tbodytr" #> threads.map(thread => {
        <tr>
          <td>
            <a href={"/forumpost/" + thread._id.toString}>
              {thread.title}
            </a>
            <br/> <span class="fullname">
            {thread.authorName}
          </span>
            <span class="date">
              {Formater.formatTime(new Date(thread._id.getTime()))}
            </span>
          </td>
          <td>
            {thread.count.toString}
          </td>
          <td>
            {Unparsed(thread.lastInfo)}
          </td>
        </tr>
      })
  }
