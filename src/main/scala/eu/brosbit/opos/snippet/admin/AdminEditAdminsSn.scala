...
import _root_.eu.brosbit.opos.lib.MailConfig
...
class AdminEditAdminsSn {
  ...
  def showSecretariat() = {
    ...
    "tr" #> secretariatUsers.map(user => {
      <tr class={if (user.validated.is) "normal" else "scratched"} ondblclick="edit(this)" title={UserChangeList.findAll(By(UserChangeList.user, user), OrderBy(UserChangeList.date, Ascending)).map(changeList => {
