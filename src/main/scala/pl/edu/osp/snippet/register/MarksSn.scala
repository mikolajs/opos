...
class MarksSn extends BaseTeacher {
  ...
  def showMarks() = {
    if (idSub == "-1") {
      ...
      "thead tr" #> <tr>
        <th>Nr</th> <th>Nazwisko i imię</th>{}<th>Średnia</th> <th>Semestr</th>
      </tr> &
        "tbody tr" #> <tr></tr>
  ...
  def showSubjects() = {
    ...
    "input" #> SubjectName.findAll(By(SubjectName.scratched, false)).map(s => {
      val checked = if (subject.id.get == s.id.get) "checked" else ""
      val radio = "radio" + s.id.get.toString
        <input type="radio" id={radio} checked={checked} onclick="marks.redirectSubject(this)" name="radio"/> ++ <label for={radio}>
        {s.name}
      </label>
    })
