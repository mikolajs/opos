...
class ForumPostSn extends UsersOperations with UpdateMainPageInfo with ForumBaseMenu {
  ...
  def showThread() = {
    ...
    "#threadtitle" #> threadHead.title &
      ...
      ".firstcomment *" #> <td>
        <div>
          {Unparsed(threadContent.content)}
        </div> <hr/>
        <p>
          <span class="fullname">
            {threadHead.authorName}
          </span>
          <span class="date">
            {Formater.formatDate(new Date(threadHead._id.getTime()))}
          </span>
          <span>
            {editNode}
          </span>
      ...
      ".comments" #> threadContent.comments.map(comment => {
        "div *" #> Unparsed(comment.content) &
          ...
          "a" #> {
            if (isAdmin) {
              <a href={"/forumpost/" + threadHead._id.toString + "?clr=" + comment.id.toString}>
  ...
  def addComment(): CssSel = {
    ...
    "#id" #> SHtml.text(threadHead._id.toString, x => x, "type" -> "hidden") &
      "#content" #> SHtml.textarea(content, x => {
        println(content); content = x.trim
      }) &
