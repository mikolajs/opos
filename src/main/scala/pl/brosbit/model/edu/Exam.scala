...
object Exam extends MongoDocumentMeta[Exam] {
  ...
  def create = new Exam(ObjectId.get, 0L, "", 0L, "", 0L, "",  Nil, 0L, 0L, false)
}

case class Exam(var _id: ObjectId, var authorId: Long,  var description: String,
                    var subjectId: Long, var subjectName: String,
                    var classId: Long, var className:String, var quizzes: List[String],
                    var start: Long, var end: Long, var multi: Boolean)
